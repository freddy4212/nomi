...
class WebcamSource:
    ...
    def detect_cameras(self) -> List[Dict]:
        """偵測可用的攝像頭"""
        cameras = []
        self.debug_log("正在掃描可用攝像頭...")
        
        # 掃描前 10 個索引，不輕易中斷，確保能抓到所有裝置
        for i in range(10):
            try:
                # 使用預設後端開啟
                ...
                if cap.isOpened():
                    ...
                    cameras.append({'id': i, 'resolution': f"{w}x{h}", 'backend': backend})
                    self.debug_log(f"找到攝像頭 {i}: {w}x{h} ({backend})")
                    cap.release()
                
                # 稍微等待，讓系統釋放資源
                time.sleep(0.1)
            except Exception as e:
                self.debug_log(f"掃描攝像頭 {i} 時發生錯誤: {str(e)}")
                
        if not cameras:
            self.debug_log("未找到任何可用攝像頭")
        else:
            self.debug_log(f"掃描完成，共找到 {len(cameras)} 個攝像頭")
