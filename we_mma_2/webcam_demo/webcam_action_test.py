#!/usr/bin/env python3
"""
webcam_action_test.py - 使用電腦攝像頭測試 MMAction2 動作辨識

功能：
- 使用電腦攝像頭即時捕捉影像
- 使用 YOLO-Pose 提取骨架關鍵點
- 使用 MMAction2 PoseC3D 進行動作辨識
- 可調整輸入 FPS 來模擬 WiseEye2 的低幀率環境
- 四種視圖模式：Original / Overlay / YOLO Only / Interpolated
- 顯示骨架、動作辨識結果

使用方式：
    conda activate we_mma_2
    python webcam_action_test.py
"""
...
class WebcamActionApp:
    def __init__(self):
        ...
        print("=" * 50)
        print("  Webcam Action Recognition Test (Unified Logic)")
        print("  使用 YOLO-Pose + MMAction2")
        print("=" * 50)
        print("\n正在初始化...")
    ...
    def start_camera(self) -> bool:
        ...
        if not self.cap.isOpened():
            print(f"⚠ 無法開啟攝像頭 {self.current_camera_id}")
            return False
        ...
        print(f"✓ 攝像頭 {self.current_camera_id} 已開啟: {local_config.camera_width}x{local_config.camera_height}")
        return True
    
    def detect_cameras(self) -> List[dict]:
        """偵測可用的攝像頭"""
        cameras = []
        for i in range(10):  # 檢查 0-9
    ...
    def refresh_cameras(self):
        ...
        print(f"✓ 偵測到 {len(self.available_cameras)} 個攝像頭")
        
        # 更新下拉選單
        if hasattr(self, 'camera_combo'):
            camera_options = [f"{c['id']}: {c['resolution']}" for c in self.available_cameras]
            camera_options.append("── 重新偵測 ──")
    ...
    def _render_static_view(self, frame: np.ndarray) -> np.ndarray:
        """渲染靜態視圖（非補幀模式）"""
        if frame is None:
            return np.zeros((local_config.camera_height, local_config.camera_width, 3), dtype=np.uint8)
    ...
    def _draw_info_overlay(self, img: np.ndarray) -> np.ndarray:
        ...
        info = {
            ...
            'output_fps': 30, # MMAction2 固定 30 FPS
            'buffer': len(self.skeleton_processor.raw_buffer),
    ...
    def _get_current_display(self) -> np.ndarray:
        ...
        if self.view_mode == ViewMode.INTERPOLATED:
            ...
            # 其他視圖：使用一卡一卡的顯示（與採樣 FPS 同步）
            if self.display_output is not None:
                output = self.display_output.copy()
            elif self.display_frame is not None:
                output = self._render_static_view(self.display_frame)
            else:
                output = np.zeros((h, w, 3), dtype=np.uint8)
    ...
    def update_labels(self):
        ...
        if result:
            # 根據模式選擇顯示的標籤
            if config.simplified.labels and hasattr(result, 'simplified_label') and result.simplified_label:
                self.current_action = result.simplified_label
            else:
                self.current_action = result.action_label
        ...
        self.confidence_var.set(f"信心度: {self.current_confidence:.1%}")
        
        # 更新骨架可見性狀態 (從本地狀態)
        is_valid, avg_conf, visible_count = self.skeleton_quality
        # 這裡簡單顯示，因為 analyze_visibility 返回的 dict 結構可能不同
        # 我們可以從 skeleton_processor 獲取更詳細的資訊，或者簡化顯示
        
        # 重新獲取詳細可見性資訊
        vis = self.skeleton_processor.analyze_visibility(0)
        if vis['is_sitting_likely']:
            status = f"🪑 可能坐著 (上:{vis['upper_visible']}/11 下:{vis['lower_visible']}/6)"
        elif vis['is_full_body']:
            status = f"🧍 全身可見 (上:{vis['upper_visible']}/11 下:{vis['lower_visible']}/6)"
        else:
            status = f"👤 部分可見 (上:{vis['upper_visible']}/11 下:{vis['lower_visible']}/6)"
        self.skeleton_status_var.set(status)
        
        # 更新動作強度狀態
        motion_type = "-"
        if self.motion_magnitude < config.motion.threshold_low:
            motion_type = "靜止"
        elif self.motion_magnitude > config.motion.threshold_high:
            motion_type = "劇烈"
        else:
            motion_type = "移動"
            
        self.motion_status_var.set(f"強度: {self.motion_magnitude:.1f} ({motion_type})")
        
        top5_text = "\n".join([f"{i+1}. {label}: {score:.1%}" 
                               for i, (label, score) in enumerate(self.top5_predictions[:5])])
        self.top5_var.set(top5_text if top5_text else "等待識別...")
        ...
        if "重新偵測" in selected:
            ...
            for cam in self.available_cameras:
                if cam['id'] == self.current_camera_id:
                    self.camera_var.set(f"{cam['id']}: {cam['resolution']}")
                    break
            return
    ...
    def run(self):
        ...
        self.root = tk.Tk()
        self.root.title("Webcam Action Recognition - YOLO-Pose + MMAction2 (Unified)")
        ...
        # === 視圖模式 + 相機選擇（並排）===
        top_control_frame = ttk.Frame(right_frame)
        top_control_frame.pack(fill=tk.X, pady=5)
        
        # 視圖模式下拉選單
        view_frame = ttk.LabelFrame(top_control_frame, text="視圖模式", padding="5")
        view_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        
        self.view_mode_var = tk.StringVar(value=ViewMode.OVERLAY.value)
        view_options = [mode.value for mode in ViewMode]
        self.view_combo = ttk.Combobox(
            view_frame, textvariable=self.view_mode_var,
            values=view_options, state='readonly', width=12
        )
        self.view_combo.pack(fill=tk.X)
        self.view_combo.bind('<<ComboboxSelected>>', self.on_view_mode_change)
        
        # 相機選擇下拉選單
        camera_frame = ttk.LabelFrame(top_control_frame, text="攝像頭", padding="5")
        camera_frame.pack(side=tk.LEFT, fill=tk.X, expand=True)
        ...
        ttk.Label(result_frame, textvariable=self.skeleton_status_var,
                 font=('SF Pro Display', 9)).pack(pady=2)
        
        # 動作強度狀態
        self.motion_status_var = tk.StringVar(value="強度: -")
        ttk.Label(result_frame, textvariable=self.motion_status_var,
                 font=('SF Pro Display', 9)).pack(pady=2)
        ...
        print("\n✓ GUI 已啟動")
        print("  - 使用右側按鈕切換視圖模式")
        print("  - 調整滑桿控制畫面更新 FPS（與 MMAction2 輸入同步）")
        print("  - 例如 FPS=2 時，畫面每 0.5 秒更新一次\n")
