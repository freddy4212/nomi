...
│      ↓          │  │      ↓        │  │  │ SQL 查詢能力        │   │
│ we_mma_sender   │  │ PostgreSQL    │→ │  │ 自然語言推理        │   │
│      ↓ (TCP)    │  │ + TimescaleDB │  │  │ 智慧助理回應        │   │
│ we_mma_receiver │→ │ + pgvector    │  │  └─────────────────────┘   │
│ (主要感知層)     │  │               │  │                             │
│      ↓          │  └───────────────┘  │                             │
│ memory_bridge   │                     │                             │
└─────────────────┴─────────────────────┴─────────────────────────────┘
...
| 模組 | 位置 | 說明 |
|------|------|------|
| **we_mma_receiver** | `we_mma_receiver/` | **主要感知層**，透過網路接收骨架資料 |
| **we_mma_2** | `we_mma_2/` | 核心框架，包含動作識別、骨架處理等核心功能 |
| **we_mma_sender** | `we_mma_sender/` | 發送端，從攝影機/測試來源發送骨架資料 |
| **memory_bridge** | `we_mma_2/memory_bridge.py` | 橋接模組，連接感知層與記憶層 |
| **home_agent_memory** | `home_agent_memory/` | 記憶層核心，資料庫管理與持久化 |

> **注意**：未來 `we_mma_2` 的核心功能將併入 `we_mma_receiver`，簡化架構。
...
```bash
cd home_agent_memory/container
./start.sh
```
...
編輯 `home_agent_memory/config.py`：
...
| **🗄 記憶層: 未安裝** | ⚪ 灰色 | `home_agent_memory` 模組不存在 |
...
```python
from home_agent_memory import create_memory_system
...
- **`we_mma_receiver/main.py`**（主要）- 網路接收模式
...
```python
from home_agent_memory.config import (
    MemoryConfig, 
    DatabaseConfig, 
    QueueConfig,
    RetentionConfig
...
- [ ] **異常偵測**：偵測異常行為模式並自動標記
- [ ] **合併 we_mma_2 核心功能到 we_mma_receiver**：簡化架構

## 疑難排解

### 找不到 home_agent_memory 模組
